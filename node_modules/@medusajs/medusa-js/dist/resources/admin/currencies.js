"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var qs_1 = __importDefault(require("qs"));
var base_1 = __importDefault(require("../base"));
var AdminCurrenciesResource = /** @class */ (function (_super) {
    __extends(AdminCurrenciesResource, _super);
    function AdminCurrenciesResource() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    /**
     * @description Lists currencies.
     * @experimental This feature is under development and may change in the future.
     * To use this feature please enable featureflag `tax_inclusive_pricing` in your medusa backend project.
     * @param payload optional
     * @param customHeaders
     * @returns the list of currencies as well as the pagination properties.
     */
    AdminCurrenciesResource.prototype.list = function (query, customHeaders) {
        if (customHeaders === void 0) { customHeaders = {}; }
        var path = "/admin/currencies";
        if (query) {
            var queryString = qs_1.default.stringify(query);
            path += "?".concat(queryString);
        }
        return this.client.request("GET", path, undefined, {}, customHeaders);
    };
    /**
     * @description Updates a currency
     * @experimental This feature is under development and may change in the future.
     * To use this feature please enable featureflag `tax_inclusive_pricing` in your medusa backend project.
     * @param code code of the currency to update.
     * @param payload update to apply to currency.
     * @param customHeaders
     * @returns the updated currency.
     */
    AdminCurrenciesResource.prototype.update = function (code, payload, customHeaders) {
        if (customHeaders === void 0) { customHeaders = {}; }
        var path = "/admin/currencies/".concat(code);
        return this.client.request("POST", path, payload, {}, customHeaders);
    };
    return AdminCurrenciesResource;
}(base_1.default));
exports.default = AdminCurrenciesResource;
//# sourceMappingURL=currencies.js.map